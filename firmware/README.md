...
The device half of the photo frame. See the [repo README](../README.md) for how the
pieces fit together, and [`../server/`](../server/) for the thing it talks to.
...
`loop()` is unreachable. See `src/app_state_machine.cpp`.
...
`pf_image_format.h` is the contract; `../tools/encode_image.py` is the
...
```bash
~/.platformio/penv/bin/pio run -d . -e photoframe            # build
~/.platformio/penv/bin/pio run -d . -e photoframe -t upload  # flash over USB
~/.platformio/penv/bin/pio device monitor               # USB CDC console
```

`cp src/secrets.h.example src/secrets.h` and fill it in first.
...
**0 — toolchain.** `pio run -d . -e bringup -t upload`, then watch the console. It asserts
...
Lives in [`../server/`](../server/), deployed from the `kube-saturn` repo. It exposes
exactly one endpoint this firmware cares about:

```
GET /latest.pfrm     ETag + Content-Length, honours If-None-Match
```

`../tools/encode_image.py` runs the identical pipeline offline, so you can produce a
blob by hand and serve it from a laptop without any of the cluster being involved.
